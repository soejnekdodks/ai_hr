...
async def handle_vacancy_file(message: Message, bot: Bot):
    ...
    await message.answer(
        f"✅ Вакансия `{file_name}` загружена. Текст сохранён. Теперь загрузите ZIP-архив с резюме."
    )
...
async def handle_unknown_document(message: Message):
    ...
    file_name = message.document.file_name
    await message.answer(
        f"❌ Файл `{file_name}` не подходит. Нужен файл вакансии (.txt/.pdf) или ZIP-архив с резюме (.zip)."
    )
...
async def handle_text(message: Message):
    ...
    """
    await message.answer(
        "📎 Я работаю только с файлами. Пожалуйста, загрузите файл вакансии (.txt или .pdf) или ZIP-архив с резюме."
    )
